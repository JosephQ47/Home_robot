...
## 毫米波顶替激光雷达：障碍源可选 + 实机入口补齐（2026-09-07）

用户目标：不做 SLAM（没有激光雷达），毫米波顶替，识别物体，RViz 给点跑一段。

**发现实机入口缺一环**：`robot_bringup.launch.py` 里**没有 Collision Monitor**。
`hr_local_motion` 发 `/cmd_vel_auto`，但没有任何节点把它转成 `/cmd_vel` ——
也就是说，实机入口即使把所有安全开关都打开，底盘也一步不动。已补进去。

**Collision Monitor 新增 `obstacle_source` 参数**（默认 `scan`，不改变既有行为）：

- `scan`：二维激光扫描，技术方案 §3.5 的正式方案。
- `hmmd`：毫米波 `/hmmd/detection`，**只在没有激光雷达时临时替代**。

`hmmd` 模式的判据是「有人且距离 ≤ stop_distance 就停」，失效安全与 scan 一致：
无数据停、距离为 NaN（未标定）也停——读不到距离绝不能当成安全。

**刻意没有把 HMMD 合成假的 LaserScan**。那样 Nav2 与代价地图会「看起来能跑」，
但整张代价地图建立在不存在的观测上，比明着跑不起来危险得多。

节点启动时会打一条 WARN 说明能力边界：毫米波只检测人体微动，**看不见墙、桌腿、
椅子**（官方称「纯静止不支持」「无法确保检测非人体移动物体」）。**它不是避障，
只是「有人靠太近就停」。** 这条写在文档里没人看，写在启动日志里躲不掉。

**新增 `upper/tools/verify_hmmd_gate.py`**，五条判据：无数据停、人太近停、
距离 NaN 停、无人放行且限幅、人在远处放行。连跑三次全 PASS。该脚本不启动
`hr_bridge`、不打开任何设备，不可能驱动底盘。

### verify_mock_framework 偶发失败的真实原因

此前判断为「资源竞争导致结果超时」，**是错的**。把错误信息拆成"超时"与"跑完但
失败"两条之后，一次失败直接给出 `outcome='REJECTED'` —— 任务是被准入检查拒绝的。

根因：测试没等 `/robot_status` 就绪就提交任务。任务准入要求状态新鲜且
`safety_permit` 为真，抢在第一条 RobotStatus 之前提交必然被拒。已补等待，
连跑四次全过。

**这条值得记**：把两种失败合并成一句话，会让人往错误方向查很久。

### 目录结构对齐技术方案 §9.1

| 方案要求 | 此前 | 处置 |
|---|---|---|
| 根 `tools/` | 缺失 | 已建，`pc_control/` 迁入（跨端串口协议工具，正是方案对 tools/ 的定义） |
| `docs/hardware/` | 缺失 | 已建，附 README 列出已实测但散在别处的硬件事实与仍为 TBD 的项 |
| `tests/integration/` | 缺失 | 已建，附 README 登记现有六个集成测试的实际位置 |
| `lower/` | — | **与方案 §9.3 完全一致**：Core/Drivers/Middlewares/BSP/Devices/Components/Protocol/Application 全在 |

**仍与方案不符、待人裁决的四项**（都没有明确的方案归属，不擅自移）：

| 目录 | 内容 | 问题 |
|---|---|---|
| `config/udev/` | 三个设备的 udev 规则 | 方案把 udev 放在各包内（如 `hr_camera/udev/`），但这些规则跨三个设备；且被 16 处引用，迁移churn 大 |
| `web_control/` | PC 侧网页后端 | 方案里网页归 `hr_web_ui`（在 upper/src），这是独立的 PC 侧件 |
| `firmware/` | 厂商固件与安全副本 | 方案的下位机是 `lower/`（C 板），OpenCTR 是另一块板的参考资料，方案无归属 |
| `hi3516/` | 相机板交叉编译与文档 | 方案 §3.2 只把 Hi3516 当相机，其 SDK 工作无归属 |

另有 `upper/tools/` 的六个验收脚本：按方案应归 `tests/integration/`，但它们全部
依赖 ROS 2 运行时，放上位机工作空间内运行更自然。已在
`tests/integration/README.md` 登记，迁不迁待人拍板——不擅自再做一次大范围改路径。
